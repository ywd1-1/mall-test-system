...
- `api.py` 中的普通函数封装获取 Token、生成请求头和选择测试商品等公共接口动作。
- `conftest.py` 中的 fixture 负责提供认证请求头、动态商品 ID 和收货地址 ID。
- `test_cart.py` 使用 `yield` fixture 组合加购、返回购物车数据和测试后删除。
- `test_orders.py` 使用普通函数组合创建、查询和取消订单，并通过 `try...finally` 保证清理。
- `test_*.py` 测试函数只保留当前场景的请求、业务断言和结果校验。
